...
The adapters report activity through the same local CLI used by the menu-bar app. They intentionally do not infer task completion from file activity. The agent must call `update-task` with a stable todo ID and evidence when a task is actually complete.

Build the CLI, then set `TT_TRACKER_TRAPPER_BIN` to the absolute binary path and `TT_RUN_ID` to a run returned by `start-run`. Copy the matching JSON into the agent's local hook/settings configuration and replace the placeholder paths and run ID with an environment-aware wrapper for your setup. Review and trust hooks in Codex before enabling them.
