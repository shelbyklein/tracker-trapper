...
- [x] **TT-05 — Implement the CLI and MCP interface.** Provide register_plan, get_plan, start_task, complete_task, report_blocker and run lifecycle operations through shared service validation. Check: both interfaces update the same records, reject invalid IDs/revisions, and return useful errors; unauthorized local requests fail. Evidence: commit `73ac597c75f5ce11c6c3d2e8d803ac40a4fc0098` provides the CLI; commit pending includes the stdio MCP server and a live initialize/tools-list protocol check. The current local transport is intentionally stdio-scoped; no network listener is exposed.
