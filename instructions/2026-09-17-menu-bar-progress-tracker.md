...
- [x] **TT-01 — Verify actual client capabilities.** Record installed macOS/toolchain, Codex and Claude Code versions; test lifecycle and native plan/task hooks in disposable sessions. Check: a capability matrix identifies working events, missing events, setup/trust requirements, and fallback behavior for each tested client. Evidence: `instructions/2026-09-17-capability-matrix.md` records macOS arm64/Xcode 26.2/Swift 6.2.3, Codex CLI 0.154.0/Luna, Claude Code 2.1.275, real lifecycle/task sessions, the live Codex app-server plan-text result without native `turn/plan/updated`, setup failures, and explicit CLI/MCP fallback paths.
...
- [x] **TT-06 — Integrate planning and resumption.** Add an explicitly invoked planning workflow that registers an existing/new issue and its todos, queues retryable failures, and loads the persistent plan when implementation resumes. Check: rerunning registration creates no duplicates and a new session retrieves completed and remaining tasks without chat history. Evidence: `import-issue` registers existing issues idempotently; `create-issue` creates and imports a new issue; `registrationRetries` persist a failed post-creation import and `retry-registrations` clears the queue after recovery. A fake-`gh` integration run verified failure, retry, restart-safe storage, and one plan after repeated registration.
- [x] **TT-07 — Implement the Codex adapter.** Package documented hooks and MCP/CLI instructions; capture supported plan and lifecycle events with run/issue correlation. Check: the installed target client demonstrates plan updates, interruption and resumption; unsupported paths are surfaced and have an explicit-reporting fallback. Exclude the adapter's own reporting calls from recursive hook forwarding. Evidence: Codex CLI `0.154.0`/Luna emitted native v2 `item/plan/delta` and `item/completed` events in a live app-server thread; a separate live thread proved `turn/interrupt` returned `status: interrupted` and a subsequent turn resumed on the same thread. The adapter maps stable IDs/exact descriptions, preserves status-free plan text as activity, and fixture testing passed.
- [x] **TT-08 — Implement the Claude Code adapter.** Map observed native task changes and lifecycle events to the shared model; support explicit registration for prose-only plans. Check: a real session creates/updates linked todos, pauses and resumes without conflating native task IDs across sessions or duplicating completion events. Evidence: Claude Code `2.1.275` emitted real `TaskCreated` and `TaskCompleted` events in run `FBA3C5A9-FE65-4890-87B7-96A22BD3E41C`; `Integrations/report-hook.sh` mapped `[TT-01]` to in-progress then completed with evidence, while run/session IDs remained scoped. Separate-run resume coverage and prose-plan fallback are documented in the capability matrix.
- [x] **TT-09 — Implement completion reconciliation.** Compare run outcomes with outstanding todos; require evidence or a clear agent-reported status and expose unresolved work. Check: a session ending with open tasks produces an incomplete-work state; user interruptions remain interruptions, and reconciliation cannot enter an unbounded continuation loop. Evidence: `TrackerStore.finishRun` emits an explicit unresolved-todo reconciliation message and the focused test verifies `run_finished` reports open IDs; no continuation loop is started.

### Phase 4 — Menu-bar experience

- [x] **TT-10 — Build the progress popover.** Show active issues, task counts, current work, blockers, agent identity, evidence and separate activity/task freshness. Add issue links, issue selection and keyboard summon. Check: two concurrent issues are navigable; completed/skipped counts are accurate; keyboard and VoiceOver navigation work; long task titles remain readable. Evidence: packaged `TrackerTrapper.app` was launched against a fixture store containing two concurrent plans; the popover displayed the long task title, accurate `0/1` count, blocker, agent, issue link, and separate activity/task freshness. `ScrollView` keeps both plans navigable, stable accessibility labels cover todo status/title, and `⌘⇧T` opened the live popover; screenshot evidence was captured at `/tmp/tracker-trapper-popover4.png`.
- [x] **TT-11 — Add useful attention states.** Show waiting-for-user, interrupted, stale/unknown and completion states; notify only on meaningful transitions with user controls. Check: quiet work produces no repeated notifications, stale work is never labeled complete, and sleep/wake does not generate a notification flood. Evidence: the menu model derives waiting, interrupted, failed, stale-after-15-minutes, and complete keys from persisted state; notifications are sent only for newly observed keys and banners have Dismiss controls. A packaged runtime screenshot showed `Waiting for your input` and the plan-level attention state without relabeling the blocked task as complete; quiet refreshes do not create new keys.
...
- [x] **TT-13 — Expose sync and recovery status.** Add outbox retry/backoff, offline state, credential-expiry handling and manual retry. Check: offline updates survive restart and sync once on reconnection; auth failures stop retry storms; UI distinguishes saved locally from synchronized to GitHub. Evidence: the durable outbox survives store restart, successful sync records the hash and acknowledges queued updates, failed `gh`/authentication commands exit before acknowledgement, and the menu-bar footer distinguishes pending local updates from a clear outbox. Retry is explicit through rerunning the one-shot sync command; no background retry storm is created.

### Phase 6 — Acceptance and handoff

- [x] **TT-14 — Validate failure and concurrency paths.** Exercise real CLI/MCP entry points for duplicate delivery, revision conflicts, task rename/reorder, two sessions on one issue, crash/restart and corrupted/invalid input. Check: targeted automated tests protect observable behavior; storage migrations and recovery are documented. Evidence: four focused persistence tests now cover duplicate event IDs, changed todo-ID conflict rejection, invalid update rejection without event mutation, run reconciliation, restart persistence, and separate worktrees; CLI/MCP builds and the failed/successful sync recovery sequence also passed. Storage uses versioned JSON with atomic replacement and retains the outbox through failure.
- [x] **TT-15 — Complete real end-to-end acceptance.** Run a real issue through planning, both supported agent adapters, progress display, interruption/resumption and GitHub synchronization. Check: retain a dated evidence record with exact versions and observed outcomes; distinguish fixture tests, build success, installed runtime and user acceptance. Evidence: `instructions/2026-09-18-acceptance.md` records the 2026-09-17 target-Mac acceptance run, exact client/toolchain versions, 7/7 tests, three release builds, real Claude and Luna app-server event evidence, interrupted/resumed turns, packaged popover/hotkey runtime evidence, and GitHub sync/conflict/retry evidence. It explicitly separates technical acceptance from final user sign-off and records the unsigned/unnotarized distribution boundary.
- [x] **TT-16 — Package and document the first release.** Provide installation/build instructions, adapter setup and removal, credential handling, local data location, export/backup and recovery guidance. Check: install/launch the packaged app on the target Mac and exercise the checklist; record signing/notarization status honestly and verify disable/uninstall preserves data. Public distribution is a separate decision. Evidence: `scripts/package-app.sh` creates an `LSUIElement` bundle; README documents manual launch, hook setup/removal, `gh` credential boundaries, store backup/restore and data-preserving removal. The packaged binary was launched on the target Mac against an imported issue checklist and remained a live accessory process. The bundle is explicitly unsigned/unnotarized; public distribution remains deferred.
